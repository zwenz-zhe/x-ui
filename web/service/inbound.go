...
func (s *InboundService) DeleteExpiredInbounds() error {
	db := database.GetDB()
	now := time.Now()
	return db.Where("expiry_time < ?", now).Delete(&model.Inbound{}).Error
}
